  ...
  return (
    <>
      <div className=" hidden lg:block sticky top-0 left-0 right-0 w-full bg-white z-50 border-b border-gray-100">
        <nav className="w-full flex items-center justify-between px-6 lg:px-10 xl:px-32 py-4 h-[80px] md:h-auto">
          ...
          <div className="flex items-center">
            ...
            <div className="hidden md:flex items-center gap-8 text-[15px] font-medium text-gray-800">
              ...
              <Link to="/profile" className="flex items-center justify-center w-10 h-10 rounded-full bg-black text-white transition-colors ml-2">
                <User size={20} />
              </Link>
            </div>
      ...
      {/* Mobile Sidebar */}
      <div
        className={`fixed top-0 left-0 h-full w-[80%] max-w-sm bg-white z-[70] transform transition-transform duration-300 ease-in-out flex flex-col lg:hidden ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'
          }`}
      >
        {/* Sidebar Header */}
